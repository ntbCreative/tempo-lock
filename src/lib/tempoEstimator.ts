...
export function estimateTempo(
  ...
  crossValidationBpm?: number | null
): TempoEstimate {
  ...
  // If a reference tempo is available and it closely matches some
  // candidate that's a harmonic (half/double/third) of the top one,
  // prefer that candidate outright -- resolving octave ambiguity using an
  // external reference is stronger evidence than any purely acoustic
  // heuristic. Scoped to genuine octave relationships (not just "closer
  // to the reference") so this doesn't override a real, different tempo
  // reading; it only steps in when the ambiguity is specifically a
  // quarter-note-vs-half/double-time question. Checked against two
  // independent references in order of trust: an explicit one the person
  // gave us (typically Tap Tempo, set just before listening starts) or
  // the currently displayed tempo, then -- new here -- the autocorrelation
  // cross-check (see autocorrelation.ts): a structurally different
  // algorithm run on the same onsets, so its agreement or disagreement
  // with the interval-clustering approach above is independent evidence,
  // not just a second opinion from the same method.
  for (const reference of [priorBpm, crossValidationBpm]) {
    if (reference == null || reference <= 0) continue;
    const referenceMatch = candidates.find((c) => Math.abs(c.bpm - reference) / reference < 0.04);
    if (!referenceMatch || referenceMatch === top) continue;
    const ratio = referenceMatch.bpm > top.bpm ? referenceMatch.bpm / top.bpm : top.bpm / referenceMatch.bpm;
    const isOctaveOfTop = Math.abs(ratio - 2) < 0.15 || Math.abs(ratio - 3) < 0.15;
    // Require the matching candidate to have *meaningful* support of its
    // own, not just happen to sit near the reference -- otherwise a truly
    ...
    const hasMeaningfulSupport = referenceMatch.score >= top.score * 0.4;
    if (isOctaveOfTop && hasMeaningfulSupport) {
      chosen = referenceMatch;
      break;
    }
  }
